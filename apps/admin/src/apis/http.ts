...
import { ApiResponse, ITokens } from '@/types';
import { ACCESS_TOKEN, KEEP_ME_LOGGED_IN, REFRESH_TOKEN } from '@/utils/constants';
...
export const refreshTokens = async () => {
  try {
    ...
    const res: ApiResponse<{ tokens: ITokens }> = await response.json();

    if (!response.ok) throw new Error(res.message);

    const { accessToken, refreshToken } = res.data.tokens;

    if (localStorage.getItem(KEEP_ME_LOGGED_IN)) {
